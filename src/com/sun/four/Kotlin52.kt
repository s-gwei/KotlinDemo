...
fun main() {
    ...
    // 这个是属于具名函数
    // str.run(具名函数)run函数持有this 都可以很灵活的，把上一个结果值 自动给 下一个函数
    str
        .run(::isLong) // this == str本身，返回为boolean，所以传给下个参数就是boolean
        .run(::showText) // this == isLong返回的boolean值，返回为String
        .run(::mapText)//参数为String，返回为string
        .run(::println)

    println()

    // let函数持有it，run函数持有this 都可以很灵活的，把上一个结果值 自动给 下一个函数
    // ::将str作为参数传到isLong方法中去
